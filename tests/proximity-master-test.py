import os, re, wave, pyaudio, contextlib, csv, spacy, pyttsx3, random, serial, glob
import numpy as np
...
def runLoop(mode="title", logic=None, playback_mode="all", recognizer="google", userInput=None):
    print("------------------------------")
    ...
    if userInput is not None:
        recognized_speech.append(userInput)
        if len (recognized_speech) > 5:
            del recognized_speech[0]
        print("stored speech : ", recognized_speech)
...
def finishedPlaybackMsg():
    arduino.write(b"FINISHED")

def arduinoPlay():
    print("arduino IR triggering playback")
    runLoop(mode="line", userInput=recognized_speech[-1])
    finishedPlaybackMsg()

if __name__ == "__main__":
    recognized_speech = ["hello world"]
